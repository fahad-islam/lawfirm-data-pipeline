...
const fetchNextRecord = Effect.gen(function* () {
    ...
    const record = yield* db.googlePlaceUrlToScrape.findFirst({
        where: {
            status: {
                equals: null
            }
...
const processOneRecord = Effect.gen(function* () {
    ...
    yield* Effect.log("✅ Found record to process", { ...record })

    yield* Effect.gen(function* () {
        yield* Effect.log("🚀 Starting workflow execution...")

        const isCompleted = yield* PlacesLocatorWorkflow.execute({
            id: record.id,
            location: record.location!,
            url: record.url
        }).pipe(
            Effect.timeout(Duration.minutes(60)),
...
mainWithMetrics.pipe(
    Effect.scoped,
    Effect.provide(PlacesLocatorWorkflowLayer),
